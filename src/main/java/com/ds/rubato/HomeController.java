...
import com.ds.rubato.dao.BoardDao;
...
public class HomeController {
	...
	public String board_writeOk(HttpServletRequest request, @RequestPart MultipartFile uploadFiles) throws Exception {
		...
		if(uploadFiles.isEmpty()) {//파일 첨부여부를 판단( true or false)
			...
			String fileurl = "D:\\springboot_workspace\\rubatoHomepage0803\\src\\main\\resources\\static\\uploadfiles\\";
			...
			int boardnum = fbDtos.get(0).getFbnum();//가져온 게시글 목록 중에서 가장 최근에 만들어진 글이 불러와짐
	...
	public String replyOk(HttpServletRequest request , Model model) {
		...
		model.addAttribute("boardId",boardDao.fbviewDao(boardnum).getFbid());
